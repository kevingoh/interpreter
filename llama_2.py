...
class LocalOllama:
    ...
    def completion_ (self, prompt):
        return completion(model="codellama", messages=prompt, api_base="http://localhost:11434", custom_llm_provider="ollama", stream=True, temperature=self.temperature)
    
    async def wait_response(self, generator):
        r = []
        async for elem in generator:
            r += [elem]
        return r
    
    async def wait_task(self, generator):
        self.response = await self.wait_response(generator)

    def get_response(self, messages):
        generator = self.completion_(messages)
        asyncio.run(self.wait_task(generator))
        return self.response
        
def get_llama_2_instance(temperature):
    return LocalOllama(temperature)
