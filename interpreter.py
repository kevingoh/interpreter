...
class Interpreter:
  ...
  def chat(self, message=None, return_messages=False):
    ...
    if self.local:
      ...
      if self.llama_instance == None:
        ...
        try:
          self.llama_instance = get_llama_2_instance(self.temperature)
        except:
  ...
  def respond(self):
    ...
    if not self.local:
      ...
      """
      response = self.llama_instance.get_response(messages)

      #task = asyncio.asyncio.create_task(get_response(response_))
      #response = await get_response(response_)
    ...
    for chunk in response:
      ...
      if not self.local:
        fc_finished = chunk["choices"][0]["finish_reason"] or llama_function_call_finished
      else:
        fc_finished = llama_function_call_finished
      ...
      if fc_finished:
        ...
        if not self.local:
          fc_fin2 = chunk["choices"][0]["finish_reason"] == "function_call" or llama_function_call_finished
        else:
          fc_fin2 = llama_function_call_finished
        ...
        if not self.local:
          endreason = chunk["choices"][0]["finish_reason"] != "function_call"
        else:
          endreason = True
          # Done!
